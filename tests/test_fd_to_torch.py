...
def firedrake_function(u):
    x = fd.SpatialCoordinate(mesh)
    F = fd.Function(V)
    F.interpolate(fd.sin(u * fd.pi) * fd.sin(x[1] * fd.pi))
    return F


templates = (fd.Function(V), fd.Constant(0.0), fd.Constant(0.0))

inputs = (torch.ones(V.dim(), requires_grad=True),
...
def test_autograd_fecr_integration():
    ...
    grad_x, = torch.autograd.grad(yy, inputs_2,
                                  grad_outputs=grads,
                                  create_graph=True)
