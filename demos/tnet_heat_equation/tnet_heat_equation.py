...
class NeuralNetwork(nn.Module):
    def __init__(self, neurons, num_observation, num_train, num_truncated_series, mc_loss_fn):
        super(NeuralNetwork, self).__init__()
        self.num_observation = num_observation
        self.num_train = num_train
        self.compute_mc_loss = mc_loss_fn
    ...
    def solverTorch(self, kappa, train_observations):
        ...
        with MPIPoolExecutor() as executor:
            losses = executor.map(self.compute_mc_loss, zip(train_observations, kappa))
            for l in losses:
                loss_mc += l
        
        # for (u_obs_true_, kappa_) in zip(train_observations, kappa):
        #     u_ = diff_solver(kappa_)[obs_indices].squeeze()

        #     loss_mc += torch.mean(torch.square(u_ - u_obs_true_)) * self.num_observation / self.num_train
...
if __name__ == '__main__':
    ...
    def compute_mc_loss(u_obs_true, k):
        u = diff_solver(k)[obs_indices].squeeze()
        loss = torch.mean(torch.square(u - u_obs_true)) * num_observation / num_train
        return loss

    model = NeuralNetwork(neurons, num_observation, num_train, num_truncated_series, compute_mc_loss).to(device)
