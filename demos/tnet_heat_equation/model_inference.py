...
def plot_u(u, u_pred, i):
    ...
    ax = plt.gca()
    ax.set_aspect("equal")
    l = fd.tricontourf(from_numpy(np.reshape(u[i, :], (256, 1)), fd.Function(V)), axes=ax, levels=levels)
    ...
    ax = plt.gca()
    ax.set_aspect("equal")
    l = fd.tricontourf(from_numpy(np.reshape(u_pred[i, :], (256, 1)), fd.Function(V)), axes=ax, levels=levels)
    fd.triplot(mesh, axes=ax, interior_kw=dict(alpha=0.05))
    plt.colorbar(l, fraction=0.046, pad=0.04)
